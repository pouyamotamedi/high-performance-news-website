...
func (h *GoogleNewsHandlers) HandleGoogleNewsSitemap(c *gin.Context) {
	// Get language from query parameter or default to "fa"
	...
	if err != nil {
		c.String(http.StatusInternalServerError, "Failed to generate Google News sitemap")
		return
	}

	// Validate the sitemap
	if err := h.sitemapService.ValidateGoogleNewsSitemap(xmlData); err != nil {
		c.String(http.StatusInternalServerError, "Invalid Google News sitemap generated")
		return
	}
