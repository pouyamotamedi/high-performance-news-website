...
// Verify2FA verifies a 2FA code and marks the session as verified
func (m *TwoFactorMiddleware) Verify2FA(c *gin.Context, userID uint64, code string) bool {
	// In a real implementation, this would validate against TOTP/SMS
	// For now, we'll use a simple time-based validation
	expectedCode := m.generateTimeBasedCode(userID)
...
// generateTimeBasedCode generates a simple time-based code (in production, use TOTP)
func (m *TwoFactorMiddleware) generateTimeBasedCode(userID uint64) string {
	// Simple time-based code for demonstration
	// In production, use proper TOTP implementation
	timeSlot := time.Now().Unix() / 30 // 30-second window
	return fmt.Sprintf("%06d", (userID+uint64(timeSlot))%1000000)
}
