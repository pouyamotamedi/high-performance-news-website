...
func (h *ThemeHandlers) UpdateTheme(c *gin.Context) {
	...
	// First, fetch the existing theme to preserve is_active and is_default flags
	existingTheme, err := h.themeService.GetTheme(id)
	if err != nil {
		if err.Error() == "theme not found" {
			c.JSON(http.StatusNotFound, gin.H{"error": "Theme not found"})
			return
		}
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to get existing theme", "details": err.Error()})
		return
	}
	...
	// Preserve the ID and important flags from existing theme
	theme.ID = id
	theme.IsActive = existingTheme.IsActive     // Preserve is_active flag
	theme.IsDefault = existingTheme.IsDefault   // Preserve is_default flag
	theme.CreatedAt = existingTheme.CreatedAt   // Preserve created_at
