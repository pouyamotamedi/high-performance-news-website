...
func (r *ThemeRepository) GetActive() (*models.Theme, error) {
	...
	log.Printf("DEBUG ThemeRepository.GetActive: executing query")
	...
	if err != nil {
		if err == sql.ErrNoRows {
			log.Printf("DEBUG ThemeRepository.GetActive: no active theme found")
			return nil, fmt.Errorf("no active theme found")
		}
		log.Printf("DEBUG ThemeRepository.GetActive: error=%v", err)
		return nil, fmt.Errorf("failed to get active theme: %w", err)
	}

	if err := json.Unmarshal(configJSON, &theme.Config); err != nil {
		log.Printf("DEBUG ThemeRepository.GetActive: unmarshal error=%v", err)
		return nil, fmt.Errorf("failed to unmarshal config: %w", err)
	}

	log.Printf("DEBUG ThemeRepository.GetActive: returning theme ID=%d, Name=%s", theme.ID, theme.Name)
	return theme, nil
}
