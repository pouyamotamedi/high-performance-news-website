...
type Comment struct {
	...
	SpamScore    float64       `json:"-" db:"spam_score"` // Hidden from JSON
...
// DetectSpam performs basic spam detection on a comment
func DetectSpam(comment *Comment) *CommentSpamDetection {
	...
	if linkCount > 2 {
		detection.Score += 0.3
		detection.Reasons = append(detection.Reasons, "excessive links")
		detection.Checks["excessive_links"] = true
	}
	...
	detection.IsSpam = detection.Score >= 0.5
