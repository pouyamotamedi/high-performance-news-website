...
// SEOData contains SEO-related metadata for articles
type SEOData struct {
...
// Scan implements the sql.Scanner interface for SEOData
...
// Value implements the driver.Valuer interface for SEOData
...
func ValidateArticle(article *Article) error {
	...
	// SEO data validation
	if err := ValidateSEOData(&article.SEOData); err != nil {
		errors = append(errors, fmt.Sprintf("seo_data: %s", err.Error()))
	}
...
// ValidateSEOData validates SEO metadata
func ValidateSEOData(seo *SEOData) error {
	...
	if seo.SchemaType != "" {
		...
		seo.SchemaType = "NewsArticle" // Default value
	}
...
func (a *Article) PrepareForDB() {
	...
	if a.SEOData.SchemaType == "" {
		a.SEOData.SchemaType = "NewsArticle"
	}
	if a.LanguageCode == "" {
