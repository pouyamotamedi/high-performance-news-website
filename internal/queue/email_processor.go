...
func (p *EmailProcessor) Start(ctx context.Context) error {
	...
	p.jobQueue.RegisterHandler(&EmailJobHandler{processor: p})

	// Start processing jobs
	return p.jobQueue.Start(ctx)
}

// EmailJobHandler implements JobHandler for email jobs
...
func (h *EmailJobHandler) Handle(ctx context.Context, job *Job) error {
	...
	return h.processor.handleEmailJob(ctx, jobData)
}

func (h *EmailJobHandler) GetJobType() JobType {
	return "email"
}
...
func (p *EmailProcessor) QueueWelcomeEmail(subscriberID uint64) error {
	...
	return p.queueJob(job, PriorityHigh)
}
...
func (p *EmailProcessor) QueueCampaignSend(campaignID uint64) error {
	...
	return p.queueJob(job, PriorityMedium)
}
...
func (p *EmailProcessor) QueueBulkEmails(emails []BulkEmail) error {
	...
	for i := 0; i < len(emails); i += batchSize {
		...
		if err := p.queueJob(job, PriorityMedium); err != nil {
...
func (p *EmailProcessor) QueueNewsletterSend(subject, content string, templateID *uint64) error {
	...
	return p.queueJob(job, PriorityLow)
}
...
func (p *EmailProcessor) handleNewsletterSend(ctx context.Context, payload map[string]interface{}) error {
	...
	if tid, exists := payload["template_id"]; exists && tid != nil {
		if tidFloat, ok := tid.(float64); ok {
			templateIDStr = fmt.Sprintf("%d", uint64(tidFloat))
		}
	}
	...
	for _, subscriber := range subscribers {
		...
		if templateIDStr != "" {
			metadata["template_id"] = templateIDStr
		}
...
func (p *EmailProcessor) queueJob(emailJob *EmailJob, priority Priority) error {
