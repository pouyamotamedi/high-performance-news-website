...
// QRadarConnector implements IBM QRadar integration
...
// SentinelConnector implements Microsoft Sentinel integration
	...
	for _, attackPath := range threatModel.AttackPaths {
		event := SecurityEvent{
			...
			Metadata: map[string]interface{}{
				...
				"steps_count":          len(attackPath.Steps),
			},
		}
		events = append(events, event)
	}
...
func (s *SIEMIntegration) executeAutoResponses(ctx context.Context, alerts []SecurityAlert) []AutoResponse {
	...
	for _, alert := range alerts {
		if alert.Severity == "critical" || alert.Severity == "high" {
			...
			if sourceIP := s.extractSourceIPFromAlert(alert); sourceIP != "" {
...
func (s *SIEMIntegration) generateAlertRecommendations(correlation CorrelationResult) []string {
	...
	case "privilege_escalation_sequence":
		...
		recommendations = append(recommendations, "Check for unauthorized system changes")
	case "data_exfiltration_pattern":
		...
		recommendations = append(recommendations, "Consider implementing DLP controls")
	default:
		recommendations = append(recommendations, "Investigate security event correlation")
		recommendations = append(recommendations, "Review system logs for additional indicators")
	}

	return recommendations
}

func (s *SIEMIntegration) extractSourceIPFromAlert(alert SecurityAlert) string {
