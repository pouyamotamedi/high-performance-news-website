...
// analyzeMetrics performs analysis on collected metrics
func (ihm *InfrastructureHealthMonitor) analyzeMetrics() {
	ihm.mu.RLock()
	...
	for name, metric := range ihm.metrics {
		if metric.Status == HealthStatusWarning || metric.Status == HealthStatusCritical {
			log.Printf("Health alert: %s is %s (value: %.2f %s)", 
				name, metric.Status, metric.Value, metric.Unit)
		}
	}
}
