...
func TestInfrastructureHealthMonitor(t *testing.T) {
	// Create health monitor
	ihm := NewInfrastructureHealthMonitor()
	
	// Start the monitor
	ctx := context.Background()
	if err := ihm.Start(ctx); err != nil {
		t.Fatalf("Failed to start infrastructure health monitor: %v", err)
	}
	defer ihm.Stop()
	...
	if score < 0 || score > 100 {
		t.Errorf("Health score should be between 0-100, got: %.2f", score)
	}
	
	// Test that metrics are being collected
	// This is a basic test since the actual metrics are simulated
	if score == 0 {
		t.Error("Health score should not be zero with default metrics")
	}
}
...
func TestErrorRecoveryIntegration(t *testing.T) {
	...
	// Test integration scenario: Database failure leading to emergency procedures
	t.Run("DatabaseFailureToEmergency", func(t *testing.T) {
		// Simulate database failure
		failure := FailureEvent{
			ID:        "integration_db_failure",
			Type:      FailureTypeDatabase,
			Severity:  SeverityCritical,
			Component: "database",
		...
		err := ers.recoverDatabaseConnection(context.Background(), failure)
		if err != nil {
			// If recovery fails, trigger emergency procedures
			emergencyEvent := EmergencyEvent{
				ID:        "emergency_db_failure",
				Type:      EmergencyTriggerCriticalFailure,
				Severity:  EmergencySeverityP0,
				Component: "database",
				Message:   "Database recovery failed, triggering emergency procedures",
				Context: map[string]interface{}{
					"original_failure": failure.ID,
				},
				Timestamp: time.Now(),
			}
			
			err = epm.TriggerEmergencyProcedure(emergencyEvent)
			if err != nil {
				t.Errorf("Emergency procedure should have been triggered: %v", err)
			}
		}
	})
	
	// Test disaster recovery scenario
	t.Run("DisasterRecoveryScenario", func(t *testing.T) {
		// Test disaster recovery plan
		err := drm.TestRecoveryPlan("database_recovery_plan")
		if err != nil {
			t.Errorf("Disaster recovery plan test failed: %v", err)
		}
		
		// Initiate actual recovery
		err = drm.InitiateRecovery("database_failure")
		if err != nil {
			t.Errorf("Failed to initiate disaster recovery: %v", err)
		}
		...
		status := drm.GetRecoveryStatus()
		if status == nil || status.Status != RecoveryStatusInProgress {
			t.Error("Disaster recovery should be in progress")
		}
	})
}
