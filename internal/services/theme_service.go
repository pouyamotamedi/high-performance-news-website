...
func (s *ThemeService) GetActiveTheme() (*models.Theme, error) {
	...
	if cached, err := s.getCachedTheme(cacheKey); err == nil && cached != nil {
		fmt.Printf("DEBUG GetActiveTheme: returning cached theme ID=%d\n", cached.ID)
		return cached, nil
	}
	...
	fmt.Printf("DEBUG GetActiveTheme: fetched from DB theme ID=%d, Name=%s, IsActive=%v\n", theme.ID, theme.Name, theme.IsActive)
