...
type MonitoringIntegrationService struct {
	...
	runbooksService    *OperationalRunbooksService
...
func NewMonitoringIntegrationService(
	...
	runbooksService := NewOperationalRunbooksService(db, config, metricsService, alertingService, remediationService)
	...
	return &MonitoringIntegrationService{
		...
		runbooksService:    runbooksService,
		db:                 db,
		cache:              cache,
		config:             config,
...
func (mis *MonitoringIntegrationService) GetSystemStatus() (*SystemStatus, error) {
	...
	recentRunbooks, err := mis.runbooksService.GetRunbookExecutions(10)
	if err != nil {
		log.Printf("Warning: Could not get runbook executions: %v", err)
...
func (mis *MonitoringIntegrationService) GetOperationalRunbooks() map[string]*OperationalRunbook {
	return mis.runbooksService.GetRunbooks()
}
