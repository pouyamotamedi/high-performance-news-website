    ...
    try {
        ...
        if (response.ok) {
            ...
            if (article.language_code) {
                document.getElementById('languageCode').value = article.language_code;
            }
        ...
        if (article) {
            ...
            if (article.categories && Array.isArray(article.categories) && article.categories.length > 0) {
                ...
                if (originalCategory) {
                    ...
                    selectedCategories.push({ 
                        id: String(article.category_id), 
                        name: 'Category #' + article.category_id 
                    });
                }
            }
...
async function updateArticle() {
    try {
        ...
        if (languageCode) updateData.language_code = languageCode;
...
async function saveAsDraft() {
    try {
        ...
        // Prepare update data - same as updateArticle but force status to draft
        const updateData = {};
        ...
        if (response.ok) {
            // Update the status dropdown to reflect the change
            document.getElementById('status').value = 'draft';
            
            // Reload article data to refresh the UI
            await loadArticleData();
        ...
        alert('❌ Error saving as draft: ' + error.message);
    } finally {
        const saveDraftBtn = document.getElementById('saveDraftBtn');
